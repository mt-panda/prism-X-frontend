...
const formData: FormDataItem[] = [
  { name: "Banner Title", type: "text", placeholder: "Enter banner title" },
  ...
  { name: "Website", type: "text", placeholder: "Enter website" },
  { name: "Map Embed (src)", type: "text", placeholder: "Enter map URL" },
  { name: "Banner Intro", type: "textarea", placeholder: "Enter banner intro" },
  { name: "About us", type: "textarea", placeholder: "Enter about us" },
  { name: "Why us", type: "textarea", placeholder: "Enter why choose us" },
  {
    name: "Latest Projects Intro",
    type: "textarea",
    placeholder: "Enter latest projects",
  },
  { name: "Our Mission", type: "textarea", placeholder: "Enter mission" },
  {
    name: "Contact us Intro",
    type: "textarea",
  ...
  },
  {
    ...
    type: "select",
    placeholder: "Select category",
    options: ["Tech", "Health", "Finance", "Retail"],
  },
  {
    name: "Region",
    type: "select",
  ...
  },
  {
    ...
    type: "select",
    placeholder: "Select services",
    options: ["Accounting", "Tax", "Bookkeeping", "Others"],
  },
];

// ---------------- Component ----------------
const CreateListing: React.FC<CreateListingProps> = ({
// Removed unused triggerResetForm prop
  setTriggerResetForm,
}) => {
  const { enqueueSnackbar } = useSnackbar();
  const auth = useContext(AuthContext);
  ...
  const handleImageUpload = (
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = () =>
        setImages((prev: any) => ({ ...prev, [key]: reader.result as string }));
      reader.readAsDataURL(file);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      setLoading(true);
      const payload = { ...form, slug: slugify(form.title, { lower: true }) };
      const formDataToSend = new FormData();

      Object.entries(payload).forEach(([k, v]) => {
        if (Array.isArray(v)) v.forEach((val) => formDataToSend.append(k, val));
        else formDataToSend.append(k, v as string);
      });
      ...
      enqueueSnackbar(id ? "Listing updated!" : "Listing created!", {
        variant: "success",
      });
      resetForm();
  ...
  return (
    <Grid
      container
      spacing={2}
      sx={{ p: 3, overflowY: "scroll", height: "100vh" }}
    >
      {/* Logo */}
      <Grid>
        <Typography>Business Logo:</Typography>
        <Box onClick={() => document.getElementById("logoUpload")?.click()}>
      ...
      {formData.map((item) => (
        <Grid key={item.name}>
          ...
          {item.type === "select" ? (
            <Select
              multiple={item.name === "Services" || item.name === "Category"}
              value={
                item.name === "Category"
                  ? form.category
                  : item.name === "Region"
              ...
              onChange={(e) => {
                const value = e.target.value;
                if (item.name === "Category") updateForm("category", value);
                if (item.name === "Region") updateForm("region", value);
                if (item.name === "Price Range")
                  updateForm("priceRange", value);
                if (item.name === "Services") updateForm("services", value);
              }}
              fullWidth
            >
            ...
            <InputBase
              ...
              onChange={(e) => {
                ...
                if (item.name === "City") updateForm("city", e.target.value);
                if (item.name === "Area") updateForm("area", e.target.value);
                if (item.name === "Phone") updateForm("phone", e.target.value);
                if (item.name === "Street")
                  updateForm("street", e.target.value);
                if (item.name === "Website")
                  updateForm("website", e.target.value);
                if (item.name === "Map Embed (src)")
                  updateForm("mapUrl", e.target.value);
              }}
            />
          )}
        </Grid>
      ...
      <Grid>
        ...
        <ReactQuill
          theme="snow"
          value={form.desc}
          onChange={(val) => updateForm("desc", val)}
        />
      </Grid>
...
// ---------------- Wrapper ----------------
export default function EnhancedCreateListing(props: any) {
  return (
